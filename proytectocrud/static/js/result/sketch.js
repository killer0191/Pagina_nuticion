...
function createButtons() {
  ...
  submitJsonModal = select("#submitJsonModal");
  submitJsonModal.mousePressed(submitJsonModalHandler);

}
function submitJsonModalHandler() {
  var PathFile = document.getElementById("fileJsonModal").value;
  if (PathFile) {
    knnClassifier.load('myKNN.json' || PathFile, customModalReady);
  } else {
...
function gotResults(err, result) {
  ...
  if (result.confidencesByLabel) {
    ...
    var max = 0;
    var lable = "";
    if (confidences["B"] < confidences["A"]) {
      max = confidences["A"] * 100;
      lable = "A";
    } else {
      max = confidences["B"] * 100;
      lable = "B";
    }
